...
1. **Pick the partition first** — on Rikyu the partition fixes the resource share per node. Use `get_facility` for the table. Rules of thumb:
...
- Don't guess Rikyu-specific details — use `search_docs` from the rikyu-docs server.
