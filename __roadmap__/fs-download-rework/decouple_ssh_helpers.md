...
WHY: remotemanager builds an rsync transport and version-checks it (≥3.0.0) at `Computer` construction, so any host with older/BSD rsync can't even run `.cmd()`. Our transfer layer only needs plain SSH exec + a checksum; a direct `ssh` subprocess is simpler, dependency-light, and portable. This is **defense-in-depth, NOT a revocation of the rsync ≥3.0 requirement**: `remotemanager` still needs ≥3.0 for `rm_rsync` and for the legacy `run_command` path used by every other `fs_*` tool, so ≥3.0 remains a deployment requirement (enforced via MCP-server env/PATH config — see the `deploy_rsync_env` productionize leaf). Decoupling just means our download exec+checksum keeps working even where that enforcement hasn't happened.
